...
describe('useSireneSearch', () => {
  ...
  it('sets error key on 404 response', async () => {
    ...
    expect(result.current.error).toBe('beneficiaries.search.errors.404');
    expect(result.current.result).toBeNull();
  });
  ...
  it('sets error key on 400 response', async () => {
    ...
    expect(result.current.error).toBe('beneficiaries.search.errors.400');
  });
  ...
  it('sets network error key when no response status', async () => {
    ...
    expect(result.current.error).toBe('beneficiaries.search.errors.network');
  });
