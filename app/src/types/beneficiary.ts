/**
 * Possible verification states for a beneficiary IBAN.
 * - PENDING: just added, not yet validated
 * - FORMAT_VALID: IBAN format is syntactically correct
 * - VERIFIED: VOP verification matched the beneficiary name
 ...
 */
export interface BeneficiaryIbanDto {
...
/**
 * A beneficiary (organisation or individual) linked to an association.
 * Each beneficiary can have multiple IBANs.
 */
export interface BeneficiaryDto {
  ...
  active: boolean;
  /** IBANs associated with this beneficiary. */
  ibans: BeneficiaryIbanDto[];
 ...
 * Contains only the fields needed for the beneficiary creation flow.
 */
...
/**
 * Payload for `POST /api/association/beneficiaries`.
 */
export interface CreateBeneficiaryRequest {
  ...
  postalCode?: string;
  /** Whether the beneficiary is active (defaults to true on the backend). */
  active?: boolean;
}

/**
 * Payload for `POST /api/association/beneficiaries/:id/ibans`.
 */
