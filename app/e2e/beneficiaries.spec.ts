...
test.describe('Beneficiaries page', () => {
  ...
  test('Beneficiaries page loads for association user', async ({ page, context }) => {
    ...
    await page.route(`${API_BASE}/api/association/beneficiaries`, (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify([]),
    ...
    await page.goto('/fr/dashboard/association/beneficiaries');

    await expect(page).toHaveURL(/\/fr\/dashboard\/association\/beneficiaries/, { timeout: 10000 });
  ...
  test('Sidebar link is active on beneficiaries page', async ({ page, context }) => {
    await setAuthCookies(context, 'ASSOCIATION', mockAssoUser.id);
    ...
    const beneficiariesLink = page.locator('a[href*="beneficiaries"]').filter({ hasText: '🏦' });
    await expect(beneficiariesLink).toBeVisible({ timeout: 10000 });
    await expect(beneficiariesLink).toHaveClass(/bg-green\/10/);
  });
});
