...
| Vérification du format et du titulaire d'un IBAN de bénéficiaire de paiement *(ajouté le 6 septembre 2026)* | Association, à l'ajout d'un IBAN puis au déclenchement de la vérification VOP | Automatique sur événement (format) · geste humain (déclenchement VOP) | Bloquant pour la sélection en paiement — seul un IBAN au statut vérifié et actif apparaît dans le formulaire ; **non opérant en production tant que le jeton d'API n'est pas remplacé**, et **en local comme en staging le statut vérifié résulte d'une simulation** et non d'une vérification bancaire : voir `verification-payee-iban.md` points 4.1, 4.3 et 4.4 (hors dispositif LCB-FT) | Association (corriger l'IBAN ou relancer la vérification) |
| Autorisation bancaire du décaissement par l'association *(ajouté le 8 septembre 2026)* | Association, à la confirmation d'un décaissement | Geste humain — authentification forte auprès de sa propre banque | Bloquant de fait lorsque l'exécution réelle est activée — le virement est alors initié via Bridge en Open Banking : l'association est le débiteur et aucun mouvement de fonds n'a lieu sans son authentification bancaire. **En production, l'exécution réelle n'est pas encore activée** : les identifiants Bridge de production ne sont pas provisionnés, `app.bridge.demo-mode` vaut `true` par défaut (`BRIDGE_DEMO_MODE`) et le décaissement est alors **simulé** — aucun appel à Bridge, aucune authentification bancaire, aucun mouvement de fonds, et le règlement est marqué acquis sans confirmation d'une banque. Le contrôle décrit ici ne vaut donc qu'une fois `BRIDGE_DEMO_MODE=false` et les identifiants posés sur l'environnement de production. Tant que ce n'est pas le cas, l'interface de production désactive le bouton d'émission d'un paiement et affiche « Les paiements ne sont pas encore activés » : aucune association ne peut déclencher un décaissement simulé depuis l'application. La condition est calculée par le serveur (`paymentsEnabled`) et combine le mode démo et le profil d'exécution — seule la production est fermée, les environnements locaux et de pré-production conservent le parcours simulé. Le contrôle n'est pas seulement d'interface : le serveur refuse (409) la création comme la confirmation d'un décaissement dans la même condition, de sorte qu'une requête rejouée directement sur l'API échoue au lieu d'être simulée. Les fonds vont directement de son compte vers l'IBAN du bénéficiaire ; ni Bridge ni CommonLink ne détiennent les fonds. L'attestation on-chain n'est publiée qu'après règlement effectif constaté par la banque. *(complété le 23 septembre 2026)* Deux issues sont distinguées : un **refus bancaire** est terminal et le lien d'autorisation est alors révoqué auprès de Bridge avant que le montant ne soit rendu au solde de la campagne, car Bridge laisse utilisable un lien refusé ; un **abandon** — lien expiré ou révoqué sans autorisation — n'est pas un échec, aucun mouvement n'ayant pu avoir lieu : le décaissement redevient réémettable et son montant retourne au solde. La durée de vie du lien est configurable par environnement (`app.bridge.link-validity`, un jour par défaut) et borne la durée pendant laquelle un montant reste engagé sans autorisation (hors dispositif LCB-FT) | Association (autoriser ou abandonner le virement auprès de sa banque ; un abandon est sans conséquence et le décaissement peut être réémis) |
...
| **Seuil de rapprochement** — score composite phonétique + orthographique par blocs (DoubleMetaphone × Jaro-Winkler) au seuil de 0,85. Le seuil a été calibré pour l'ancien Jaro-Winkler plein-chaîne ; une recalibration empirique sur le registre en production est recommandée. Abaisser le rappel d'un criblage de sanctions est une décision juridique. | Volume de faux positifs soumis au responsable de la conformité |
