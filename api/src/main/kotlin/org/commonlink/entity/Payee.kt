 ...
 *
 * A payee is always owned by one [AssociationProfile] and identified by its French SIREN
 * ([identifier1], 9 digits). The optional [identifier2] stores the SIRET (14 digits) when the
 * specific establishment is known.
 *
 * Each payee can hold multiple verified IBANs through the [ibans] collection. Only IBANs
...
class Payee(
    ...
    /** The association that manages this payee.*/
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "association_id", nullable = false)
    val association: AssociationProfile,

    /** Official registered name of the payee organisation. */
    ...
    /** NAF/APE activity code describing the payee's sector of activity. */
    @Column(name = "activity_code", length = 10)
    var activityCode: String? = null,

    /** Legal or administrative category of the payee (e.g. association loi 1901). */
    @Column(name = "category", length = 100)
    var category: String? = null,

    /** City where the payee is headquartered. */
    @Column(name = "city", length = 255)
    var city: String? = null,

    /** Postal code of the payee's registered address. */
    @Column(name = "postal_code", length = 10)
    var postalCode: String? = null,

    /** Whether this payee is currently active and eligible to receive donations. */
    ...
    /**
     * Collection of IBAN entries associated with this payee.
     *
     * Managed via cascade: adding or removing from this list persists automatically.
     * Orphaned [PayeeIban] records are deleted when removed from the collection.
     */
    @OneToMany(mappedBy = "payee", cascade = [CascadeType.ALL], orphanRemoval = true)
    val ibans: MutableList<PayeeIban> = mutableListOf()
}
