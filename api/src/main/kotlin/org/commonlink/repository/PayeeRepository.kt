...
interface PayeeRepository : JpaRepository<Payee, UUID> {
    /**
     * Returns all payees belonging to the given association, ordered by persistence order.
     *
     * @param associationId the UUID of the [org.commonlink.entity.AssociationProfile]
     */
    fun findAllByAssociationId(associationId: UUID): List<Payee>

    /**
     * Finds a payee by its own ID and the owning association's ID.
     *
     * Used to verify ownership before returning or mutating data — prevents cross-association access.
     *
     * @param id the UUID of the [Payee]
     * @param associationId the UUID of the [org.commonlink.entity.AssociationProfile]
     */
    fun findByIdAndAssociationId(id: UUID, associationId: UUID): Optional<Payee>

    /**
     * Checks whether a payee with the given SIREN already exists under the association.
     *
