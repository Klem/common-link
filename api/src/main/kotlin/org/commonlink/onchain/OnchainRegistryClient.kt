...
class OnchainRegistryClient(
    ...
    // ───── Curator writes ───────────────────────────────────────────
    override fun verifyAssociation(address: String, sirenHash: ByteArray): TransactionReceipt =
        curatorContract.verifyAssociation(address, sirenHash).send()

    override fun revokeAssociation(address: String): TransactionReceipt =
        curatorContract.revokeAssociation(address).send()

    override fun restoreAssociation(address: String): TransactionReceipt =
        curatorContract.restoreAssociation(address).send()

    override fun pauseCampaign(id: ByteArray): TransactionReceipt = curatorContract.pauseCampaign(id).send()
    override fun unpauseCampaign(id: ByteArray): TransactionReceipt = curatorContract.unpauseCampaign(id).send()
    override fun cancelCampaign(id: ByteArray): TransactionReceipt = curatorContract.cancelCampaign(id).send()
    override fun completeCampaign(id: ByteArray): TransactionReceipt = curatorContract.completeCampaign(id).send()
    override fun revertCampaignToDraft(id: ByteArray): TransactionReceipt = curatorContract.revertCampaignToDraft(id).send()
    ...
    override fun publishCampaign(id: ByteArray): TransactionReceipt = recorderContract.publishCampaign(id).send()

    /** Mutates budgetHash on a non-terminal campaign. Reverts with BudgetHashUnchanged on identical hash. */
    override fun updateCampaignBudget(id: ByteArray, newBudgetHash: ByteArray): TransactionReceipt =
    ...
    override fun markMilestoneReached(campaignId: ByteArray, index: BigInteger, proofHash: ByteArray): TransactionReceipt =
     ...
     */
    override fun recordPayout(payoutId: ByteArray, campaignId: ByteArray, amountCents: BigInteger): TransactionReceipt =
