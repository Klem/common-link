...
contract CommonLinkRegistry is AccessControl, Pausable {
    ...
    /// @dev    Storage layout (per spec §3.3):
    ///           slot 0: association (20B) + goal (12B)
    ///           slot 1: raised (12B) + startDate (4B) + endDate (4B) + status (1B) + milestoneCount (1B) — 10B free
    ///           slot 2: budgetHash (32B)
    struct Campaign {
        address association;
        ...
        bytes32 budgetHash; // keccak256 of the canonical (JCS) budget JSON
    }
    ...
    {
        if (association == address(0)) revert InvalidAssociation();
        if (sirenHash == bytes32(0)) revert EmptySirenHash();

        Association storage a = associations[association];

        if (a.verified) revert AssociationAlreadyVerified(association);
        ...
        if (a.sirenHash != bytes32(0) && a.sirenHash != sirenHash) {
            revert SirenAlreadyRegistered(a.sirenHash, association);
        }

        // Global uniqueness: this sirenHash must not already be claimed by another
        // address.
        address existing = sirenHashToAssociation[sirenHash];
        if (existing != address(0) && existing != association) {
            revert SirenAlreadyRegistered(sirenHash, existing);
        }
    ...
    function revokeAssociation(address association) external onlyRole(CURATOR_ROLE) {
        ...
        if (a.verifiedAt == 0) revert AssociationNeverVerified(association);
        if (!a.verified) revert AssociationNotVerified(association);
    ...
    function restoreAssociation(address association) external onlyRole(CURATOR_ROLE) {
        ...
        if (a.verifiedAt == 0) revert AssociationNeverVerified(association);
        if (a.verified) revert AssociationNotRevoked(association);
    ...
    function createCampaign(
        ...
        if (!associations[association].verified) revert AssociationNotVerified(association);
        if (campaigns[campaignId].association != address(0)) {
            revert CampaignAlreadyExists(campaignId);
        }
        if (goal == 0) revert InvalidGoal();
        if (endDate <= startDate) revert InvalidDateRange(startDate, endDate);
        ...
        if (endDate <= block.timestamp) revert InvalidDateRange(startDate, endDate);
        if (budgetHash == bytes32(0)) revert EmptyBudgetHash();
    ...
    function recordDonation(
        ...
        if (donationId == bytes32(0)) revert EmptyDonationId();
        if (donations[donationId].donor != address(0)) {
            revert DonationAlreadyExists(donationId);
        }
        if (donor == address(0)) revert InvalidDonor();

        Campaign storage c = campaigns[campaignId];
        if (c.association == address(0)) revert CampaignDoesNotExist(campaignId);

        // Runtime soft-revoke check: if the campaign's association has been
        // revoked, the donation is refused.
        if (!associations[c.association].verified) {
            revert AssociationNotVerified(c.association);
        }

        if (c.status != CampaignStatus.Active) {
            revert CampaignNotActive(campaignId, c.status);
        }
        ...
        // forge-lint: disable-next-line(block-timestamp)
        if (block.timestamp < c.startDate || block.timestamp > c.endDate) {
            revert CampaignNotInWindow(uint32(block.timestamp), c.startDate, c.endDate);
        }
        if (amount == 0) revert InvalidAmount();
        if (receiptHash == bytes32(0)) revert EmptyReceiptHash();
        if (txRef == bytes32(0)) revert EmptyTxRef();
    ...
    function pauseCampaign(bytes32 campaignId) external onlyRole(CURATOR_ROLE) {
        Campaign storage c = _requireCampaignExists(campaignId);
        if (c.status != CampaignStatus.Active) {
            revert InvalidStatusTransition(c.status, CampaignStatus.Paused);
        }
    ...
    function unpauseCampaign(bytes32 campaignId) external onlyRole(CURATOR_ROLE) {
        ...
        if (c.status != CampaignStatus.Paused) {
            revert InvalidStatusTransition(c.status, CampaignStatus.Active);
        }
    ...
    function cancelCampaign(bytes32 campaignId) external onlyRole(CURATOR_ROLE) {
        ...
        if (c.status == CampaignStatus.Cancelled || c.status == CampaignStatus.Completed) {
            revert CampaignAlreadyTerminal(campaignId);
        }
     ...
     */
    function markMilestoneReached(bytes32 campaignId, uint8 index, bytes32 proofHash)
    external
    whenNotPaused
    onlyRole(RECORDER_ROLE)
    {
        ...
        if (index >= c.milestoneCount) {
            revert InvalidMilestoneIndex(index, c.milestoneCount);
        }
    ...
    function _requireCampaignExists(bytes32 campaignId)
    internal
    view
    ...
    function _changeStatus(bytes32 campaignId, Campaign storage c, CampaignStatus newStatus)
    internal
    {
